...
module.exports = {
  ...
  async execute(interaction) {
    try {
      let allergens = await findOrFetchAllergens();
      let user = await findOrCreateUser(interaction);

      const userAllergenValues = new Set(user.allergens.map((allergen) => allergen.number));
      const selectedValues = new Map();

      categorizeUserAllergens(allergens, userAllergenValues, selectedValues);

      const selectMenus = createAllergenSelectMenus(allergens, userAllergenValues);
      ...
      collector.on("collect", async (i) => {
        if (i.isStringSelectMenu()) {
          ...
          if (i.customId === "submit") {
            const selectedAllergenValues = Array.from(selectedValues.values()).flat();
            const uniqueSelectedAllergenValues = [...new Set(selectedAllergenValues)]; // Ensure uniquenes
            if (
              ...
              try {
                user.allergens = uniqueSelectedAllergenValues.map((value) =>
                  allergens.find((allergen) => allergen.number.toString() === value)
                );
                await user.save();
      ...
      console.error(error);
      await interaction.reply("Es gab einen Fehler beim Anzeigen der Einstellungen.");
    }
  },
};
...
function categorizeUserAllergens(allergens, userAllergenValues, selectedValues) {
